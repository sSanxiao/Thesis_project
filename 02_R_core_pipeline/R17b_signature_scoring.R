# ============================================================
# R17b: Signature scoring + cluster/age-level analysis
# ------------------------------------------------------------
# 4 scores computed:
#   sig_94_zscore, sig_94_AMS (AddModuleScore)
#   sig_core_zscore, sig_core_AMS
#
# Outputs:
#   - Score per cell, added to Seurat object (saved as aldinger2021_scored.rds)
#   - Cluster × score boxplots (4 panels)
#   - Age × score trends
#   - Cluster × Age heatmap (the big summary figure)
#   - UMAP overlays
#   - ANOVA / Kruskal-Wallis stats
#   - Method concordance (zscore vs AMS)
# ============================================================
...
cat("================================================================\n")
cat("R17b: Signature scoring on Aldinger 2021 scRNA\n")
...
cat(sprintf("  sig_94: %d matched (%d pos + %d neg)\n",
...
cat(sprintf("  sig_core: %d matched (%d pos + %d neg)\n",
            length(match_core), length(sig_core_pos), length(sig_core_neg)))

# ============================================================
# 2. Compute z-score-weighted signature (Method 1)
# ============================================================
cat("\n[2] Method 1: z-score weighted signature score...\n")

# scale.data 是 SCT 残差, 即 z-score-like
expr <- GetAssayData(obj, assay = "RNA", layer = "scale.data")

compute_weighted_score <- function(expr, pos_genes, neg_genes) {
  # score = mean(+z for pos) - mean(+z for neg)   [方向加权]
  # 等价于 mean(sign × z)
  if (length(pos_genes) + length(neg_genes) == 0) return(NULL)
  ...
  # 合并: signed average, weighted by group size
...
cat(sprintf("  sig_94 zscore range:   [%.3f, %.3f], median=%.3f\n",
            min(score_94_zscore), max(score_94_zscore), median(score_94_zscore)))
cat(sprintf("  sig_core zscore range: [%.3f, %.3f], median=%.3f\n",
            min(score_core_zscore), max(score_core_zscore), median(score_core_zscore)))

# ============================================================
# 3. Compute AddModuleScore (Method 2)
# ============================================================
cat("\n[3] Method 2: Seurat AddModuleScore...\n")
cat("  AMS uses 'data' layer; we've clipped SCT residuals to [0, max]\n")

# AddModuleScore 需要 list of gene sets
# 对于有方向的 signature, 我们需要把 pos 和 neg 分别算然后相减
...
obj <- AddModuleScore(obj,
                      ...
                      assay = "RNA",
                      name = "sigcore_")
# 注意 sig_core 可能只有一个方向 (都是 negative)
if (length(sig_core_pos) > 0) {
  obj$sig_core_AMS <- obj$sigcore_1 - obj$sigcore_2
} else {
  obj$sig_core_AMS <- -obj$sigcore_1
}

cat(sprintf("  sig_94 AMS range:   [%.3f, %.3f]\n",
            min(obj$sig_94_AMS), max(obj$sig_94_AMS)))
cat(sprintf("  sig_core AMS range: [%.3f, %.3f]\n",
            min(obj$sig_core_AMS), max(obj$sig_core_AMS)))

# 把 zscore scores 也加到 obj
obj$sig_94_zscore <- score_94_zscore[colnames(obj)]
obj$sig_core_zscore <- score_core_zscore[colnames(obj)]

# ============================================================
# 4. Method concordance: zscore vs AMS correlation
# ============================================================
cat("\n[4] Method concordance (zscore vs AMS)...\n")
...
cat(sprintf("  sig_94:   Spearman ρ = %.3f\n", cor_94))
cat(sprintf("  sig_core: Spearman ρ = %.3f\n", cor_core))

# ============================================================
# 5. Cluster-level statistics
# ============================================================
cat("\n[5] Per-cluster signature score statistics...\n")

md <- obj@meta.data
md$sig_94_zscore <- obj$sig_94_zscore
md$sig_94_AMS <- obj$sig_94_AMS
md$sig_core_zscore <- obj$sig_core_zscore
md$sig_core_AMS <- obj$sig_core_AMS

md_dt <- as.data.table(md)

cluster_stats <- md_dt[, .(
  n = .N,
  sig_94_zscore_mean = mean(sig_94_zscore, na.rm = TRUE),
  sig_94_zscore_median = median(sig_94_zscore, na.rm = TRUE),
  sig_94_zscore_sd = sd(sig_94_zscore, na.rm = TRUE),
  sig_94_AMS_mean = mean(sig_94_AMS, na.rm = TRUE),
  sig_94_AMS_median = median(sig_94_AMS, na.rm = TRUE),
  sig_core_zscore_mean = mean(sig_core_zscore, na.rm = TRUE),
  sig_core_AMS_mean = mean(sig_core_AMS, na.rm = TRUE)
), by = cell_type][order(-sig_94_zscore_mean)]

print(cluster_stats)
fwrite(cluster_stats, file.path(RES_DIR, "cluster_score_stats.csv"))

# ANOVA + Kruskal-Wallis for each of 4 scores
cat("\n[6] ANOVA + Kruskal-Wallis per score:\n")
stats_dt <- data.table()
for (scorename in c("sig_94_zscore", "sig_94_AMS", "sig_core_zscore", "sig_core_AMS")) {
  ...
  stats_dt <- rbind(stats_dt, data.table(
    score = scorename,
    anova_p = anova_p,
...
p2 <- plot_box("sig_94_AMS", "sig_94 (AddModuleScore)")
p3 <- plot_box("sig_core_zscore", "sig_core (zscore-weighted)")
p4 <- plot_box("sig_core_AMS", "sig_core (AddModuleScore)")

combined_box <- (p1 + p2) / (p3 + p4) +
  plot_annotation(
    title = sprintf("Aldinger 2021 fetal cerebellum (n=%d cells, %d cell types)",
...
# Figure 2: Cluster × Age heatmap (sig_94 zscore as main, AMS as supplementary)
age_cluster_94 <- md_dt[, .(mean_score = mean(sig_94_zscore, na.rm = TRUE),
                              n = .N),
...
p_heat <- ggplot(age_cluster_94, aes(x = factor(age_pcw), y = cell_type_ord)) +
  ...
  labs(x = "Age (PCW)", y = NULL,
       title = "sig_94 (zscore) mean score: cluster × age",
       subtitle = "Numbers = cell counts; grey = <20 cells (undisplayed)") +
...
# Same for sig_core
age_cluster_core <- md_dt[, .(mean_score = mean(sig_core_zscore, na.rm = TRUE),
                                n = .N),
                            by = .(cell_type_ord, age_pcw)]
...
p_heat_core <- ggplot(age_cluster_core, aes(x = factor(age_pcw), y = cell_type_ord)) +
  ...
  geom_text(aes(label = ifelse(n < 20, "", sprintf("%d", n))),
            size = 2.5, color = "grey30") +
  labs(x = "Age (PCW)", y = NULL,
       title = "sig_core (zscore) mean score: cluster × age",
       subtitle = "Numbers = cell counts; grey = <20 cells") +
  theme_minimal(base_size = 12) +
  theme(axis.text.y = element_text(size = 9),
        plot.title = element_text(face = "bold"))
...
# Figure 3: UMAP with sig_94 overlay
umap_dt <- data.table(
...
# UMAP coloured by cell_type (reference)
...
conc_dt <- data.table(zscore = obj$sig_94_zscore,
                       AMS = obj$sig_94_AMS,
                       cell_type = obj$cell_type)
# subsample to 5000 cells for plotting
set.seed(42)
...
p_conc <- ggplot(conc_sub, aes(x = zscore, y = AMS)) +
  ...
  labs(x = "sig_94 (zscore-weighted)",
       ...
       title = "Method concordance (5000 cells subsampled)",
       subtitle = sprintf("Spearman ρ = %.3f (all %d cells)", cor_94, nrow(conc_dt))) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold"))
...
# ============================================================
# 8. Save scored object and summary
# ============================================================
cat("\n[8] Saving scored object...\n")
saveRDS(obj, file.path(RES_DIR, "aldinger2021_scored.rds"))
cat(sprintf("  Saved: aldinger2021_scored.rds (%.1f MB)\n",
            file.size(file.path(RES_DIR, "aldinger2021_scored.rds")) / 1e6))

# Summary
summary_lines <- c(
  "================================================================",
  "R17b Signature Scoring on Aldinger 2021 — SUMMARY",
  sprintf("Timestamp: %s", Sys.time()),
  "================================================================",
  "",
  "COHORT",
  sprintf("  Aldinger et al. 2021 Nat Neurosci fetal cerebellum scRNA"),
  sprintf("  %d cells x %d genes (UCSC Cell Browser variable gene subset)", ncol(obj), nrow(obj)),
  sprintf("  21 cell types, 10 age points (9-21 PCW)"),
  "",
  "SIGNATURE MATCHING",
  sprintf("  sig_94:   %d/%d matched (%d pos + %d neg)",
          length(match_94), length(sig_94),
          length(sig_94_pos), length(sig_94_neg)),
  sprintf("  sig_core: %d/%d matched (%d pos + %d neg)",
  ...
  "",
  "ANOVA p-values (cell type effect)"
)
for (i in 1:nrow(stats_dt)) {
  summary_lines <- c(summary_lines,
    sprintf("  %-18s p = %.2e", stats_dt$score[i], stats_dt$anova_p[i]))
}

summary_lines <- c(summary_lines, "",
  "TOP 5 CLUSTERS BY sig_94 zscore MEAN")
for (i in 1:5) {
  summary_lines <- c(summary_lines,
    sprintf("  %-15s n=%5d  sig_94_zscore_mean=%+.3f  sd=%.3f",
            cluster_stats$cell_type[i],
            cluster_stats$n[i],
            cluster_stats$sig_94_zscore_mean[i],
            cluster_stats$sig_94_zscore_sd[i]))
}

summary_lines <- c(summary_lines, "",
  "BOTTOM 5 CLUSTERS BY sig_94 zscore MEAN")
last5 <- (nrow(cluster_stats) - 4):nrow(cluster_stats)
for (i in last5) {
  summary_lines <- c(summary_lines,
    sprintf("  %-15s n=%5d  sig_94_zscore_mean=%+.3f  sd=%.3f",
            cluster_stats$cell_type[i],
...
summary_lines <- c(summary_lines, "",
  ...
  sprintf("  Scored rds: %s/aldinger2021_scored.rds", RES_DIR),
  sprintf("  Stats: %s/cluster_score_stats.csv", RES_DIR),
  sprintf("         %s/cluster_ANOVA_stats.csv", RES_DIR),
  "",
  "================================================================"
)

writeLines(summary_lines, file.path(RES_DIR, "R17b_SUMMARY.txt"))
cat(sprintf("\n  SUMMARY: %s/R17b_SUMMARY.txt\n", RES_DIR))

cat("\n================================================================\n")
cat("R17b DONE — ready for R17c (pseudotime on neuronal lineage)\n")
cat("================================================================\n")
